...
so the plan is basically to drop the variables whos missing base_salary. (I know its not ideal but yea...) so essentially the model will only me train on MLS players
...
```
```{r}
# Now that the missingness througout the dataset is 10%, i will loop throug it and replace the missing values with mean or mode depending on the type of variable.
...
On a second thought I decided to leave in the outliers. I already know there are players like Messi who has a crazy salary, however I do believe superstar coming to the MLS and having huge salaries is a real phenomenon and should be reflected in the model.
...
💡 Summary workflow after merging your dataframe:
