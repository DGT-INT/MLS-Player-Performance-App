...
vis_miss(mlsnp)
```
So i don't have any salary data for MLSNP. This is an issue. I am assuming other leagues will be missing salary too but for the purpose of using this project to display my soccer analytics abilities i will continue.
...
```
So I am missing alot of data outside the MLS. This is an issue. I will train/test the models with the data i have. I will create another dataset for app deployment so all the players can still be searched.
...
so the plan is basically to drop the variables whos missing base_salary. (I know its not ideal but yea...) so essentially the model will only be train on MLS players
...
```
based a on a visual inspection there appears to be outliers. However, I decided to leave in the outliers. I knew there are players like Messi who has a crazy salary. I do believe superstar coming to the MLS and having huge salaries is a real phenomenon and should be reflected in the model.

# Cleaning the data structure
I need to clean the structure so ensuring the variables are the right type and name.
```{r}
...
working_data2 <- working_data %>%
  ...
  rename("minutes played" = minutes_played.x,
         ...
         "average distance vertical yards" = avg_vertical_distance_yds,
         "share of team touches" = share_team_touches,
         "games played" = count_games) %>%
  select(-team_id, -minutes_played.y) %>%
  rename("shots on target" = shots_on_target,
         "expected goals" = xgoals,
         "goals - expected goals" = goals_minus_xgoals,
         "key passes" = key_passes,
         "assists" = primary_assists,
         "expected assists" = xassists,
         "primary assists - exp. assists" = primary_assists_minus_xassists,
         "exp. goals + exp. assists" = xgoals_plus_xassists,
         "points added" = points_added,
         "exp.points added" = xpoints_added,
         "goals + assists" = goals_plus_primary_assists)

glimpse(working_data2)
  # pick up from goals_minus_xgoals 
```
...
# Next steps
- Feature engineering ( off the top of my head i am thinking about goals per 90 min and assist per 90 minutes. as we move forward we will see what else comes to mind)
- EDA such as Explore correlations & distributions
- Modeling
- App Development
